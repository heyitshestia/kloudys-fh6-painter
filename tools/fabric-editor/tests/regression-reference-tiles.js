async page => page.evaluate(async () => {
  ...
  await loadOverlayImageFromUrl(source.toDataURL(), "tile-accuracy.png");
  if (overlaySampler.tiles.size || overlaySampler.canvas) throw new Error("Sampler eagerly decoded pixels");
  let samples = 0;
  ...
  for (const [x, y] of coordinates) {
    const actual = readOverlayPixel(x, y);
    ...
    if (overlaySampler.tiles.size > 32) throw new Error("Tile cache exceeded memory bound");
    samples++;
  }
  if (String(readOverlayPixel(255, 256)) !== "255,0,0,255") throw new Error("Evicted tile did not recreate exact one-pixel marker");
  const tileBytes = [...overlaySampler.tiles.values()].reduce((sum, tile) => sum + tile.data.byteLength, 0);
  const surface = overlaySampler.canvas;
  removeOverlay();
  if (overlaySampler || surface.width !== 1 || surface.height !== 1) throw new Error("Sampler backing storage not released");
