  ...
  try {
    for (const size of sizes) {
      ...
      if (size) operations.push(await timed("reference-load", async () => {
        ...
        return page.evaluate(() => ({ pixels: overlaySampler.width * overlaySampler.height, sourceBytes: overlaySourceState.dataUrl.length }));
      }));
      ...
      for (let repeat = 0; repeat < 3; repeat++) {
        ...
        operations.push(await timed("continuous-edit-and-recovery", () => page.evaluate(async repeat => {
          ...
          const source = overlaySourceState?.dataUrl;
          ...
          if (!autosaveStatus.serverOk) throw new Error("Storage benchmark recovery failed: " + autosaveStatus.error);
          ...
          return { repeat, recovery: autosaveStatus.state, browserFallback: autosaveStatus.browserOk };
        }, repeat)));
      }
      operations.push(await timed("project-reopen", () => page.evaluate(async size => {
        ...
        if (before !== JSON.stringify(sourceOverlayProjectState()) || shapes !== JSON.stringify(snapshotShapes())) throw new Error("Saved project did not reopen exactly");
        return { exact: true };
      }, size)));
      await cdp.send("HeapProfiler.collectGarbage");
      ...
      if (size === sizes.at(-1)) {
        ...
        operations.push(await timed("recovery-cpu4x", () => page.evaluate(async () => {
          ...
          if (!autosaveStatus.serverOk) throw new Error("Throttled recovery failed");
        })));
      ...
      await page.evaluate(async () => { endHybridRenderNow(); removeOverlay(); await clearAutosave(); documentDirty = false; });
      await cdp.send("HeapProfiler.collectGarbage");
