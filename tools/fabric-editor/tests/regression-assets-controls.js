async (page) => {
  ...
  const historyCount = await page.evaluate(() => history.length);
  await x.press("Tab");
  check(await page.evaluate(() => history.length) === historyCount, "Blur after Enter must not duplicate history");
  ...
  check(await page.evaluate(() => history.length) === startHistory + 1, "Label drag must be one undo step");
  ...
  return { assetWorkflow: true, numericWorkflow: true, overlapWorkflow: true, geometry };
}
