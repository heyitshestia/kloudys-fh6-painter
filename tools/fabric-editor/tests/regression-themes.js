async (page) => {
  ...
  await page.evaluate(async () => { await KfpsEditorPreferences.flush(); await clearAutosave(); documentDirty = false; });
  await page.reload();
  await page.waitForFunction(() => window.KfpsDesktop?.ready && document.documentElement.dataset.editorTheme === "custom");
  const restored = await page.evaluate(() => ({ base: document.documentElement.dataset.editorThemeBase, accent: themeFieldCurrentValues()["--accent"], matte: getComputedStyle(document.body).backgroundImage === "none" }));
  ...
  return { contrast: measures, basicColors: 6, advancedColors: 33, previewCancel: true, validation: true, reset: true, customReload: restored, responsiveWidths: [1440, 900, 360] };
}
