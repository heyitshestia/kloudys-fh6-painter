async (page, options) => {
  ...
  try {
    ...
    await timed("native-reference-load", async () => {
      ...
      return page.evaluate(() => ({ bytes: overlaySourceState.dataUrl.length, pixels: overlaySampler.width * overlaySampler.height }));
    });
    await timed("first-reference-checkpoint", () => page.evaluate(async () => {
      await flushPendingAutosave();
      if (!autosaveStatus.serverOk) throw new Error("Initial reference checkpoint was not acknowledged");
    }));
    for (const throttle of [1, 4]) {
      ...
      await timed(`background-only-cpu${throttle}`, () => page.evaluate(async () => {
        ...
        await flushPendingAutosave();
        if (!autosaveStatus.serverOk) throw new Error("Background-only checkpoint failed");
      ...
      for (let iteration = 0; iteration < 3; iteration++) {
        await timed(`steady-checkpoint-cpu${throttle}-${iteration}`, () => page.evaluate(async () => {
          ...
          await flushPendingAutosave();
          if (!autosaveStatus.serverOk) throw new Error("Background checkpoint failed");
          return { acknowledgementMs: performance.now() - started, browserOk: autosaveStatus.browserOk };
        }));
      }
    }
    ...
    const exact = await page.evaluate(async () => {
      ...
      if (JSON.stringify(recovered.shapes) !== JSON.stringify(snapshotShapes()) || recovered.editor_source_overlay.data_url !== overlaySourceState.dataUrl) throw new Error("Final checkpoint lost layers or reference bytes");
      return true;
    });
    ...
    await cdp.send("Emulation.setCPUThrottlingRate", { rate: 1 });
    await page.evaluate(async () => { removeOverlay(); await clearAutosave(); documentDirty = false; });
    await cdp.detach();
  }
}
