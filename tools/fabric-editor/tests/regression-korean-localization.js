async (page) => {
  ...
  const initial = await page.evaluate(async () => {
    ...
    reportAutosaveResult({ recovery_revision: autosaveRevision }, true, true, null);
    return {
      lang: document.documentElement.lang, title: document.title,
      family: family?.textContent, value: family?.value,
      shapeName: shapeDisplayName("Primitives", 1), displayName: localizedShapeDisplayName("Primitives", 1),
