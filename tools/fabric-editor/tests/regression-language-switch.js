async (page) => {
  ...
  for (const language of ["en", "ko"]) {
    ...
    for (const theme of ["pastel", "dark", "blackout", "whiteout"]) {
      await page.evaluate(theme => applyEditorTheme(theme, {persist: false}), theme);
    ...
    results.push({language, layout, requiredConfirmation:true, failedWritesRetry:true, persistedAcknowledgment:true, savedProject:saved.name, safeLanguageSwitch:true});
  }
  await page.evaluate(async () => {
