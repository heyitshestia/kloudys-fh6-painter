async (page) => {
  ...
  const capture = request => {
    if (request.method() === "POST" && request.url().includes("/api/fabric-editor/save-project")) writes.push(request.postDataJSON());
  };
  ...
  check(saved.shapes.length === 2, "Saved project must contain both layers");
  ...
  check(await page.evaluate(() => currentProjectName === "Enter Save As" && vinylObjects().length === 2 && !documentDirty), "Enter-saved project must reopen through Open Project");
  ...
  await page.evaluate(() => selectAllLayers());
  ...
  check(exported.shapes.length === 2 && exported.shapes.every(shape => Object.keys(shape).every(key => !key.startsWith("editor_"))), "Enter-exported JSON must contain both flat layers without editor metadata");
  ...
  await page.screenshot({ path: "enter-workflow.png" });
  return { saveAsEnter: true, reopenedThroughUi: true, savedLayers: 2, layerAndGroupRename: true, unicode: true, numpadEnter: true, duplicateProtected: true, failedSaveRetry: true, cancellation: ["Escape", "Cancel click", "Enter on Cancel"], explicitContinue: true, implicitSubmit: true, promptReplacement: true, flatExport: true, firstSaveEnter: true, singleSubmission: true, themeEnter: true, multilineUnchanged: true };
}
