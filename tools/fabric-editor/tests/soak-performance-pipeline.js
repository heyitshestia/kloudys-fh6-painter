async (page, options = {}) => {
  ...
  while (Date.now() - started < 21 * 60 * 1000) {
    ...
    const elapsed = await page.evaluate(async count => {
      ...
      if (history.length > 80 || vinylObjects().length !== 3000) throw new Error("Sustained edit changed layer/history bounds");
    ...
    if (Date.now() - started >= nextCheckpoint) {
      ...
      const state = await page.evaluate(async () => {
        ...
        return { history: historyStorageEstimate(), historyEntries: history.length, layers: vinylObjects().length,
          masks: maskPreviewOutlines.size, selectionHelpers: selectedShapeOutlineHelpers.size,
          referencePixels: (overlaySampler?.width || 0) * (overlaySampler?.height || 0),
          longTasks: window.soakLongTasks.length, maxLongTaskMs: Math.max(0, ...window.soakLongTasks), recovery: autosaveStatus.state };
      });
