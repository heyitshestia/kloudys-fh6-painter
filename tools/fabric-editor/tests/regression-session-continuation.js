async page => {
  ...
  const expected = await page.evaluate(async () => {
    ...
    await loadOverlayImageFromUrl(image.toDataURL(), "session.png");
    ...
    if (!stored.editor_session.saved || !autosaveStatus.serverOk) throw new Error("Successful Save discarded clean session recovery");
    return JSON.stringify({ shapes: snapshotShapes(), reference: sourceOverlayProjectState() });
  });
  await page.reload();
  await page.waitForFunction(() => window.KfpsDesktop?.ready);
  const clean = await page.evaluate(expected => ({
    exact: expected === JSON.stringify({ shapes: snapshotShapes(), reference: sourceOverlayProjectState() }),
  ...
  const dirtyExpected = await page.evaluate(async () => {
    ...
    const expected = JSON.stringify({ shapes: snapshotShapes(), reference: sourceOverlayProjectState() });
    documentDirty = false;
    return expected;
  });
  await page.reload();
  await page.waitForFunction(() => window.KfpsDesktop?.ready);
  const dirty = await page.evaluate(expected => ({ exact: expected === JSON.stringify({ shapes: snapshotShapes(), reference: sourceOverlayProjectState() }), dirty: documentDirty, dialog: $("autosaveRecoveryDialog").open }), dirtyExpected);
  ...
  const guarded = await page.evaluate(async () => {
    ...
    const originalRestore = restoreSourceOverlayFromProject;
    let release, entered;
    const arrived = new Promise(resolve => { entered = resolve; });
    const gate = new Promise(resolve => { release = resolve; });
    restoreSourceOverlayFromProject = async state => { await originalRestore(state); entered(); await gate; };
    try {
      const loading = loadProjectPayload({ name: "Guarded project", shapes: before.shapes.slice(0, 10), editor_source_overlay: before.editor_source_overlay });
      await arrived;
      await flushPendingAutosave();
      const during = await readAutosavePayload();
      const close = await executeDesktopOperation("close", { action: "keep-recovery" });
      if (JSON.stringify(during.shapes) !== JSON.stringify(before.shapes) || close.ok || recoveryRestoreDepth !== 1) throw new Error("Partial restore replaced checkpoint or allowed native close");
      ...
      return { partialCheckpointBlocked: true, closeBlocked: true, completeCheckpoint: true };
    } finally { release?.(); restoreSourceOverlayFromProject = originalRestore; }
  });
  checks.push(guarded);
  const failures = await page.evaluate(async () => {
    ...
    if (overlayImage || !unavailableSourceOverlayState || !documentDirty) throw new Error("Reference failure was hidden or marked clean");
    ...
    removeOverlay(); await flushPendingAutosave();
  ...
  const preparation = await page.evaluate(async () => {
    const image = document.createElement("canvas"); image.width = image.height = 16;
    image.getContext("2d").fillRect(0, 0, 16, 16);
    await loadOverlayImageFromUrl(image.toDataURL(), "preparation.png");
    currentProjectName = "Reference-only preparation";
    await saveProject(); await flushPendingAutosave();
    if (documentDirty || !autosaveStatus.serverOk) throw new Error("Reference-only project could not be saved");
    return sourceOverlayProjectState();
  });
  await page.reload();
  await page.waitForFunction(() => window.KfpsDesktop?.ready);
  assert(await page.evaluate(expected => vinylObjects().length === 0 && !documentDirty
    && JSON.stringify(sourceOverlayProjectState()) === JSON.stringify(expected), preparation), "Reference-only session was not restored");
