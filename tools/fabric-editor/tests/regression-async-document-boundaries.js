async page => {
  return page.evaluate(async () => {
    ...
    try {
      ...
      assert(!vinylObjects().length && historyIndex === 0 && !historyLocked, "History crossed a document boundary");
    ...
    await restoreSourceOverlayFromProject({ kind: "layered_svg", svg_text: svg, file_name: "Existing.svg" });
    const reference = JSON.stringify(sourceOverlayProjectState());
    addOverlayFile(new File(["<not-svg"], "Broken.svg", { type: "image/svg+xml" }));
    await new Promise(resolve => setTimeout(resolve, 50));
    assert(JSON.stringify(sourceOverlayProjectState()) === reference, "Broken SVG replacement changed the previous reference");
    results.push("invalid SVG preserves previous reference");

    const NativeReader = window.FileReader;
    let deliver;
    window.FileReader = class {
      readAsText() { this.result = svg; deliver = () => this.onload(); }
    };
    try {
      addOverlayFile(new File([svg], "Delayed.svg", { type: "image/svg+xml" }));
      removeOverlay(); deliver(); await nextFrame();
      assert(!overlayImage && !layeredOverlayState && !overlaySourceState, "A late FileReader resurrected a removed reference");
