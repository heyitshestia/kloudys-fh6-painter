  ...
  try {
    ...
    for (const viewport of [{ width: 1440, height: 900, deviceScaleFactor: 1 },
      ...
      const row = await page.evaluate(async viewport => {
        ...
        if (!beginHybridRender("high resolution test") || !hybridRenderNow()) throw new Error("GPU canvas unavailable");
        ...
        endHybridRenderNow();
        await flushPendingAutosave();
        if (!autosaveStatus.serverOk || vinylObjects().length !== 3000) throw new Error("High-resolution edit/recovery failed");
        return result;
      }, viewport);
      rows.push(row);
