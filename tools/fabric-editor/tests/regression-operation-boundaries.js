  ...
  const results = await page.evaluate(async () => {
    ...
    for (const operation of ["add", "duplicate", "paste", "replace"]) {
      ...
      try {
        ...
        results.push({ operation, preserved: vinylObjects().length === 0 && historyIndex === 0, layers: vinylObjects().length });
    ...
    results.push({ operation: "replacement installation failure", preserved: injected && failed && before === JSON.stringify(snapshotShapes()) && index === historyIndex });
