async (page) => {
  ...
  check(reopened.name === "Enter Save As" && !reopened.dirty && reopened.shapes.length === 2, "The Enter-saved project must reopen after a complete process restart");
  ...
  check(persisted.shapes.length === 2, "Restart Save As must write both layers to disk");
  return { freshProcessReopen: true, savedAgainWithEnter: true, groups: true, layers: 2, renamedLayerLabelPreserved };
}
