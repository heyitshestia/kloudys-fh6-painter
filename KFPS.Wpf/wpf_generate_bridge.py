...
def parse_args():
    parser = argparse.ArgumentParser(description="KFPS WPF headless generation bridge")
...
def main():
    ...
    print(f"WPF_RUN_DIR: {run_dir}", flush=True)
    ...
    if previews:
        print(f"WPF_PREVIEW: {previews[0]}", flush=True)
