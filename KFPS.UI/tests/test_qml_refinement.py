...
class QmlRefinementTests(unittest.TestCase):
    ...
    def test_support_page_replaces_header_promo_and_hides_after_unlock(self):
        main = self.read("Main.qml")
        host = self.read("components/CachedPageHost.qml")
        sidebar = self.read("shell/Sidebar.qml")
        ...
        self.assertIn('support: supportLoader', host)
        self.assertIn('source: "../pages/SupportPage.qml"', host)
    ...
    def test_legacy_dashboard_page_is_retired(self):
        ...
        controller = (UI / "src" / "kfps_ui" / "app_controller.py").read_text(encoding="utf-8")
        host = self.read("components/CachedPageHost.qml")
        self.assertIn('"dashboard": "create"', controller)
        self.assertIn('source: "../pages/CreatePage.qml"', host)
    ...
    def test_outputs_copy_action_also_targets_the_windows_clipboard(self):
        outputs = self.read("pages/JsonPage.qml") + self.read(
            "components/OutputExplorerContextMenus.qml"
        )
    ...
    def test_outputs_use_explorer_style_selection_and_confirmed_mixed_deletion(self):
        ...
        self.assertIn("id: outputBackgroundMouse", outputs)
        self.assertIn("z: -1", outputs)
    ...
    def test_outputs_use_a_source_scoped_file_explorer_without_changing_community_browsing(self):
        page = self.read("pages/JsonPage.qml")
        menus = self.read("components/OutputExplorerContextMenus.qml")
        outputs = page + menus
        community = self.read("pages/CommunityPage.qml")
        combo = self.read("components/KfpsComboBox.qml")
        service = (UI / "src" / "kfps_ui" / "json_service.py").read_text(encoding="utf-8")
        index = (UI / "src" / "kfps_ui" / "json_index.py").read_text(encoding="utf-8")
        self.assertIn("model: jsonService.explorerModel", outputs)
        self.assertIn("required property bool usesMasks", outputs)
        self.assertIn("required property int maskCount", outputs)
        self.assertIn('objectName: "OutputMasksBadge:" + fileCard.displayName', outputs)
        self.assertIn('text: "MASKS"', outputs)
        ...
        self.assertIn("OutputExplorerContextMenus", page)
        self.assertIn("id: contextMenu", menus)
        self.assertIn("id: deleteSelectedEntriesDialog", outputs)
        self.assertIn("onAccepted: jsonService.deleteSelectedEntries()", outputs)
        self.assertIn('property bool contextIsSource: false', menus)
        self.assertIn('contextIsSource = String(entryKind || "") === "source"', menus)
        for action in ('? "Cut "', '? "Copy "', 'text: "Rename folder"', 'text: "Rename JSON"', '? "Delete "'):
            self.assertIn(action, outputs)
        self.assertIn('text: root.contextIsFolder ? "New folder inside" : "New folder"', menus)
        self.assertIn('visible: root.contextIsFolder && !root.contextIsSource', menus)
        self.assertIn('enabled: jsonService.currentFolder.length > 0', outputs)
        self.assertNotIn("setLibraryFolderVisible", outputs)
        self.assertNotIn("jsonService.sourceIndex === 3", outputs)
        self.assertNotIn("jsonService.sourceIndex === 3", community)
        self.assertIn('model: ["Generated finals", "Editor exports", "Game exports", "Library"]', community)
        self.assertIn("jsonService.pasteIntoFolder(destination)", outputs)
        self.assertIn('objectName: "OutputSort"', outputs)
        self.assertIn('jsonService.setSortMode(modes[currentIndex])', outputs)
        self.assertIn('id: moveFolderMenu', menus)
        self.assertIn('parent: Overlay.overlay', menus)
        self.assertIn('contextMenu.parent.mapFromItem(root, sceneX, sceneY)', menus)
        self.assertIn('moveFolderMenu.parent.mapFromItem(', menus)
        self.assertIn("jsonService.prepareExplorerContextSelection(contextIndex)", page)
        self.assertIn("property int contextSelectionCount: 0", menus)
        self.assertIn("property int contextClipboardCount: 0", menus)
        self.assertIn("enabled: root.contextCanPaste", menus)
        self.assertIn("def prepareExplorerContextSelection(self, index):", service)
        self.assertIn('model: root.moveFolderModel', menus)
        self.assertIn('signal copyRequested()', menus)
        self.assertIn('onCopyRequested: jsonService.copySelection()', page)
        self.assertIn('root.moveRequested(destinationRow.path)', menus)
        self.assertIn('jsonService.moveSelectedJsonsToFolder(destination)', page)
        self.assertIn('def moveSelectedJsonsToFolder(self, value):', service)
        ...
        self.assertIn('OUTPUT_FOLDER_MARKER = ".kfps-output-folder"', index)
    ...
    def test_shell_lazily_retains_pages_after_their_first_visit(self):
        main = self.read("Main.qml")
        host = self.read("components/CachedPageHost.qml")
        livery = self.read("pages/LiveryPage.qml")
        self.assertIn("CachedPageHost {", main)
        self.assertIn("currentPage: appController.currentPage", main)
        self.assertNotIn('source: "pages/" + ({', main)
        self.assertIn("return selected || loader.item !== null", host)
        self.assertIn('visible: root.currentPage === "outputs"', host)
        self.assertIn('visible: root.currentPage === "liveries"', host)
        self.assertIn('visible: root.currentPage === "community"', host)
        self.assertIn("asynchronous: true", host)
        ...
        self.assertIn("active: root.pageActive && !root.showGallery && root.viewerSessionUrl.length > 0", livery)
